...
- `CrashFeedback`은 simulator crash와 ASAN 결과를 crash objective에 연결합니다. bare timeout은 crash artifact가 아니라 timeout telemetry로 집계합니다.
...
Crash artifact는 재현 가능해야 합니다. crash objective에 걸린 입력은 아래 위치에 저장합니다. ASAN report나 simulator crash 없이 timeout만 발생한 입력은 기본적으로 crash artifact로 저장하지 않습니다.
