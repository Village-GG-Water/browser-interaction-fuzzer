...
```json
{
  ...
  "browser_session": {
    ...
    "open_contexts_after_close": 0,
    "discarded_browser": false
  },
  "action_trace": [
    {
      "index": 0,
...
`browser_session`은 browser reuse 검증용 진단 정보입니다. `SIMULATOR_REUSE_BROWSER=1`에서 정상적으로 context 단위 격리가 유지되면 같은 simulator worker 안에서 `browser_launch_id`는 유지되고 `context_id`는 testcase마다 증가하며, 각 testcase 종료 후 `open_contexts_after_close`는 0이어야 합니다. crash-like 예외가 발생하면 `discarded_browser=true`로 보고하고 persistent browser를 폐기합니다.
